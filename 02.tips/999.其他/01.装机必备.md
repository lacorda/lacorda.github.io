...
:::danger
nvm一个很重要的作用就是 ***避免全局安装的权限问题***，但是仍然出现`EACCES: permission denied`问题，是什么原因？

1. 首先`EACCES: permission denied`报错是指权限不足
2. 最常见的原因是：目录所有权被意外修改（最常见）​​
    - 若你曾用sudo安装过全局包（如`sudo npm install -g pnpm`），会导致`~/.nvm`目录或其子目录（如`~/.nvm/versions/node/<version>/lib/node_modules`）的所有权变为`root`。后续用`普通用户`执行`npm`或`pnpm`命令时，因无写入权限而报错（`EACCES: permission denied`）

```bash
sudo chown -R <yourusername> ~/.nvm
...
# 检查npm的全局安装路径是否在~/.nvm目录下
# 若输出为/home/<your_username>/.nvm/versions/node/<version>，则说明配置正确。
